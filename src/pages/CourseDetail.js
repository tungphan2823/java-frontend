...
const CourseDetailPage = () => {
    ...
    const fetchCourseDetails = async () => {
      try {
        const response = await fetch(
          `https://student-manage-backend-82842541718a.herokuapp.com/SJC/${params.courseID}`
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const response = await fetch(
          `https://student-manage-backend-82842541718a.herokuapp.com/courses/${params.courseID}`
        );
        if (!response.ok) {
          throw new Error(`Error: ${response.status}`);
        }
