...
const StudentDetailPage = () => {
  ...
  useEffect(() => {
    const fetchStudentCourses = async () => {
      try {
        const response = await fetch(`https://student-manage-backend-82842541718a.herokuapp.com/SJC/${params.studentID}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        const response = await fetch(`https://student-manage-backend-82842541718a.herokuapp.com/students/${params.studentID}`);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
