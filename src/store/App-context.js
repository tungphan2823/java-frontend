...
export const DataProvider = ({ children }) => {
    ...
    const fetchStudents = async () => {
      try {
        const response = await fetch(
          "https://student-manage-backend-82842541718a.herokuapp.com/students/"
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await fetch(
          "https://student-manage-backend-82842541718a.herokuapp.com/courses/"
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchSJCData = async () => {
      try {
        const response = await fetch("https://student-manage-backend-82842541718a.herokuapp.com/SJC/");
        if (!response.ok) {
